...
```text Original Text
Foxtrot Lima Alpha Golf left bracket
...
![flag{abxnniohkalmcosayfiglet}](colorful_flag.png)
...
搜索得知 `.data` 段用于存放已初始化的全局变量，而 `.rodata` 用于存放常量数据，于是容易想到可以一个字符一个字符地打印。
